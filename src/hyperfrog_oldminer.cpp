...
// ------------------------------ TOPOMINE (REFERENCE) ------------------------
// Paper-aligned defaults (HF paper v33.x):
//   - s sampled uniformly from {0,1}^4096
//   - accept iff popmin <= pop(s) <= popmax and beta1(s) >= beta_min
//   - optional connectedness predicate (no normalization transform)
//
// IMPORTANT:
//   This "reference" miner intentionally avoids deterministic transforms such as
//   keep-largest-component and avoids "best-of" selection. It implements
//   predicate-only rejection sampling (parallelized as a race-to-first-accept).
static constexpr int HF_TOPOMINE_POPMIN_DEFAULT = 1200;
static constexpr int HF_TOPOMINE_POPMAX_DEFAULT = 2600;
...
#pragma omp parallel
    {
        ...
        while (!found.load(std::memory_order_relaxed)) {
            // Budget check only occasionally.
            ...
            const int wt = hf_shape_popcount(local);
            if (wt < HF_TOPOMINE_POPMIN_DEFAULT || wt > HF_TOPOMINE_POPMAX_DEFAULT) continue;

            st = hf_compute_topology(local, scratch);
            if (st.genus < beta_min) continue;
            if (require_connected && st.components != 1) continue;
...
// It is kept for benchmarking/experimentation but is not paper-aligned.
static Shape hf_mine_shape_practical(int min_genus = 8, uint64_t budget_ms = 30000) {
    Shape best{};
    HFTopo bestS{};
    std::atomic<bool> found{false};
...
static bool hf_keygen(PublicKey& pk, SecretKey& sk,
    ...
    if (!practical_miner) {
        const bool ok = hf_mine_shape_reference(sh, st, target_genus, mine_budget_ms, require_connected);
        if (!ok) {
            std::cerr << "\n" << YEL
                      << "[TopoMine(reference)] Timeout (" << mine_budget_ms
                      << " ms); no deterministic fallback. Increase --mine-ms or lower --min-genus."
                      << RST << "\n";
            return false;
        }
        ...
        std::cout << "\n" << GRN
                  << "[TopoMine(reference)] Found: genus=" << st.genus
                  << " components=" << st.components
                  << " wt=" << wt
                  << " window=[" << HF_TOPOMINE_POPMIN_DEFAULT << "," << HF_TOPOMINE_POPMAX_DEFAULT << "]"
                  << (require_connected ? " connected=1" : " connected=optional")
                  << RST << "\n";
    } else {
        // Engineering miner (normalization + best-of selection + deterministic fallback on timeout).
        sh = hf_mine_shape_practical(target_genus, mine_budget_ms);
        st = hf_analyze_shape(sh);
        std::cout << "\n" << GRN
                  << "[TopoMine(practical)] Using engineering miner output: genus=" << st.genus
                  << " components=" << st.components
                  << " wt=" << hf_shape_popcount(sh)
                  << RST << "\n";
    }
...
static void run_test_all() {
    ...
    PublicKey pk{};
    SecretKey sk{};
    if (!hf_keygen(pk, sk, /*min_genus=*/8, /*mine_budget_ms=*/30000, /*threads=*/0,
                 /*practical_miner=*/false, /*require_connected=*/false)) {
        std::cerr << RED << "[HF] KeyGen failed during self-test (TopoMine timeout)." << RST << "\n";
        return;
    }
    save_pk("test.pk", pk);
...
static int run_benchmark(const std::string& prefix,
    ...
    if (bench_keygen || !have_keys) {
        ...
        if (!hf_keygen(pk, sk, target_genus, mine_ms, threads, practical_miner, require_connected)) {
            std::cerr << "Benchmark keygen: TopoMine timeout (increase --mine-ms or lower --min-genus)\n";
            return 3;
        }
...
static void print_help(const char* argv0) {
    std::cout
        ...
        << "Usage:\n"
        << "  " << argv0 << " --gen-keys <prefix> [--password <pw>] [--mine-mode paper|practical] [--require-connected] [--mine-ms <ms>] [--threads <n>] [--min-genus <g>]\n"
        << "  " << argv0 << " --validate-keys <prefix> [--password <pw>]\n"
        << "  " << argv0 << " --enc <prefix> <in_plain> <out_hf>\n"
        << "  " << argv0 << " --dec <prefix> <in_hf> <out_plain> [--password <pw>]\n"
        << "  " << argv0 << " --test-all\n"
        << "  " << argv0 << " --benchmark <prefix> [--password <pw>] [--iters <n>] [--file-mb <mb>] [--bench-keygen] [--out <outprefix>] [--mine-mode paper|practical] [--require-connected] [--mine-ms <ms>] [--threads <n>] [--min-genus <g>]\n"
        << "  " << argv0 << " --help\n"
        << "\nFiles:\n"
        ...
        << "  Mine modes:\n"
        << "    --mine-mode paper     : rejection sampling from uniform s, then predicate checks:\n"
        << "                           popcount window [1200,2600] and beta1(s)>=min_genus (paper-aligned).\n"
        << "                           Use --require-connected to add components==1 as an extra predicate.\n"
        << "    --mine-mode practical : engineering miner (keep-largest-component + best-of selection);\n"
        << "                           includes deterministic timeout fallback (not paper-aligned).\n"
        << "  NOTE: In paper mode, timeout fails KeyGen (no deterministic fallback).\n"
        << "\nBenchmark:\n"
...
auto parse_flags = [&](size_t start_idx) -> bool {
    for (size_t i = start_idx; i < args.size(); ++i) {
        ...
        if (a == "--password" || a == "--pwd") {
            ...
            else if (*v == "paper" || *v == "reference") practical_miner = false;
            else return false;
    ...
    if (cmd == "--benchmark") {
        ...
        return run_benchmark(prefix, password, bench_iters, bench_file_mb, bench_keygen, min_genus, mine_ms, threads, bench_out, practical_miner, require_connected);
    }
    ...
    if (cmd == "--gen-keys") {
        ...
        if (!hf_keygen(pk, sk, min_genus, mine_ms, threads, practical_miner, require_connected)) {
            return die("KeyGen failed: TopoMine timeout (increase --mine-ms or lower --min-genus)." );
        }
