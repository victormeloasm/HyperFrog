...
static constexpr const char* HF_AAD_STR = "HF_V34_CONNECTED_GRAPH_FORMAL";
...
#pragma omp parallel
    {
        ...
        while (!found.load(std::memory_order_relaxed)) {
            ...
            if (st.components != 1) continue; // should hold by construction
            if (st.vertices != HF_TOPOMINE_TARGET_WEIGHT_DEFAULT) continue;
            if (st.cycle_rank < min_cycle_rank) continue;
...
static bool hf_keygen(PublicKey& pk, SecretKey& sk,
    ...
    if (!practical_miner) {
        const bool ok = hf_mine_shape_reference(sh, st, target_cycle_rank, mine_budget_ms, require_connected);
        if (!ok) {
            std::cerr << "\n" << YEL
        ...
        std::cout << "\n" << GRN
                  ...
                  << " wt=" << wt
                  << " exact_weight=" << HF_TOPOMINE_TARGET_WEIGHT_DEFAULT << " connected=1(by construction)"
                  << RST << "\n";
    } else {
...
static void run_test_all() {
    ...
    PublicKey pk{};
    SecretKey sk{};
    if (!hf_keygen(pk, sk, /*min_cycle_rank=*/HF_TOPOMINE_MIN_CYCLE_RANK_STRONG_DEFAULT, /*mine_budget_ms=*/30000, /*threads=*/0,
...
static void hf_write_bench_json(const std::string& path,
    ...
    const double dec_mbps = (file_dec_crypto_ms > 0.0) ? ((double)file_bytes / (1024.0 * 1024.0)) / (file_dec_crypto_ms / 1000.0) : 0.0;

    o << "{\n";
    o << "  \"version\": \"HyperFrog v36.0\",\n";
    o << "  \"benchmark_schema\": \"v2\",\n";
    ...
    o << "  \"file\": {\"bytes\": " << file_bytes
      ...
      << ", \"enc_MBps\": " << enc_mbps
      << ", \"dec_MBps\": " << dec_mbps
      << ", \"enc_mib_per_sec\": " << enc_mbps
      << ", \"dec_crypto_mib_per_sec\": " << dec_mbps
...
static int run_benchmark(const std::string& prefix,
    ...
    if (bench_keygen || !have_keys) {
        ...
        if (!hf_keygen(pk, sk, target_cycle_rank, mine_ms, threads, practical_miner, require_connected)) {
            std::cerr << "Benchmark keygen: TopoMine timeout (increase --mine-ms or lower --min-cycle-rank)\n";
            return 3;
        }
        ...
        ocsv << "keygen,0," << keygen_ms << ",0,\"cycle_rank=" << target_cycle_rank << ";mine_ms=" << mine_ms << ";threads=" << threads << "\"\n";
        ocsv.flush();
    } else {
...

    {
        ...
        if (!ok) {
            ...
            hf_write_bench_json(js, prefix, target_cycle_rank, threads, mine_ms, iters, file_mb, did_keygen, keygen_ms, encst, decst,
...
static void print_help(const char* argv0) {
    std::cout
        << "HyperFrog HFv6 / code version v36.0 (research prototype)\n"
        << "Usage:\n"
...
auto parse_flags = [&](size_t start_idx) -> bool {
    for (size_t i = start_idx; i < args.size(); ++i) {
        ...
        if (a == "--password" || a == "--pwd") {
        ...
        } else if (a == "--min-cycle-rank" || a == "--min-genus") {
            const std::string* v = need();
            if (!v) return false;
            ...
            else if (*v == "formal" || *v == "paper" || *v == "reference") practical_miner = false;
            else return false;
    ...
    if (cmd == "--gen-keys") {
        ...
        if (!hf_keygen(pk, sk, min_cycle_rank, mine_ms, threads, practical_miner, require_connected)) {
